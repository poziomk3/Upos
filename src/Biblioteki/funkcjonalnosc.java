...
public abstract class funkcjonalnosc {
    ...
    public static Object[][] zapelnijTabeleStudentow(ArrayList<Osoba> baza) {
        ...
        for (int i = 0; i < baza.size(); i++)
            if (baza.get(i) instanceof Student)
                wynik.add(new Object[]{i, baza.get(i).getImie(), baza.get(i).getNazwisko(), baza.get(i).getPesel(), ((Student) baza.get(i)).getNrIndeksu()});
        if (wynik.size()==0){
            return null;}
